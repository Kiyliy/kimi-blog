 ...
 */
async function fetchPublicNotionPage(pageId) {
  try {
    // 构建公开页面的URL - 必须使用完整域名
 ...
 */
async function extractNotionPageData(pageId) {
  ...
  try {
    ...
    // 2. 尝试其他可能的数据变量模式
    const dataPatterns = [
      { pattern: /window\.__INITIAL_DATA__\s*=\s*({.*?});<\/script>/s, name: 'INITIAL_DATA' },
      { pattern: /window\.__PRELOADED_STATE__\s*=\s*({.*?});<\/script>/s, name: 'PRELOADED_STATE' },
      { pattern: /window\.__INITIAL_STATE__\s*=\s*({.*?});<\/script>/s, name: 'INITIAL_STATE' },
      { pattern: /__NEXT_DATA__\s*=\s*({.*?});<\/script>/s, name: 'NEXT_DATA' },
      { pattern: /<script id="__NEXT_DATA__" type="application\/json">(.*?)<\/script>/s, name: 'NEXT_DATA_SCRIPT' }
    ];
...
module.exports = { fetchPublicNotionPage, extractNotionPageData }; 